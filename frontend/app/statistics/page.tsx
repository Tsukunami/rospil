...
const contractsChartTypes = [
  { key: "timeline", label: "Динамика договоров" },
  { key: "statusTimeline", label: "Статусы по времени" },
  { key: "statusPie", label: "Распределение по статусам" },
] as const;
...
export default function StatisticsPage() {
  ...
  const [activeTab, setActiveTab] = useState<
    "contracts" | "expenses" | "prices" | "deliveries"
  >("contracts");
  const [contractsChartType, setContractsChartType] = useState<
    "timeline" | "statusTimeline" | "statusPie"
  >("timeline");
  ...
  const timelineData = useMemo(() => {
    ...
    filteredContracts.forEach((contract) => {
      ...
      point.count++;
      point[
        contract.suppliers_contract_status as keyof Omit<
          StatsPoint,
          "date" | "count" | "expense"
        >
      ]++;
      ...
      if (
        contract.suppliers_contract_status === "оплачено" &&
        contract.suppliers_contract_cost
      ) {
        const cost =
          typeof contract.suppliers_contract_cost === "string"
  ...
  const supplierStats = useMemo(() => {
    ...
    filteredDeliveries.forEach((delivery) => {
      ...
      stats[supplier.supplier_name][
        delivery.delivery_status as keyof typeof stats[typeof supplier.supplier_name]
      ]++;
  ...
  const supplierMonthlyData = useMemo(() => {
    ...
    filteredDeliveries.forEach((delivery) => {
      ...
      monthlyData[monthLabel][
        delivery.delivery_status as keyof typeof monthlyData[typeof monthLabel]
      ]++;
    });
  ...
  const handlePrevContractsChart = () => {
    const nextIndex =
      contractsCurrentPage === 0
        ? contractsChartTypes.length - 1
        : contractsCurrentPage - 1;

    setContractsCurrentPage(nextIndex);
    setContractsChartType(
      contractsChartTypes[nextIndex].key as typeof contractsChartType
    );
  };

  const handleNextContractsChart = () => {
    const nextIndex =
      contractsCurrentPage === contractsChartTypes.length - 1
        ? 0
        : contractsCurrentPage + 1;
  ...
  const getSelectedWoodInfo = () =>
    products.find((p) => p.wood_id === selectedWoodId);
    ...
    <div className={styles.page}>
      ...
      <div className={styles.statsCards}>
        <div className={styles.statCard}>
          <div className={styles.statCardTitle}>Всего договоров</div>
          <div className={styles.statCardValue}>{totalContracts}</div>
        </div>
        <div className={styles.statCard}>
          <div className={styles.statCardTitle}>Оплачено</div>
          <div className={styles.statCardValue} style={{ color: COLORS.оплачено }}>
            {statusCounts.оплачено}
          </div>
          <div className={styles.statCardPercent}>
            {totalContracts > 0
              ? Math.round((statusCounts.оплачено / totalContracts) * 100)
              : 0}
            %
          </div>
        </div>
        <div className={styles.statCard}>
          <div className={styles.statCardTitle}>Просрочено</div>
          <div className={styles.statCardValue} style={{ color: COLORS.просрочено }}>
            {statusCounts.просрочено}
          </div>
          <div className={styles.statCardPercent}>
            {totalContracts > 0
              ? Math.round((statusCounts.просрочено / totalContracts) * 100)
              : 0}
            %
          </div>
        </div>
        <div className={styles.statCard}>
          <div className={styles.statCardTitle}>Срок не наступил</div>
          <div
            className={styles.statCardValue}
            style={{ color: COLORS["срок оплаты не наступил"] }}
          >
            {statusCounts["срок оплаты не наступил"]}
          </div>
          <div className={styles.statCardPercent}>
            {totalContracts > 0
              ? Math.round(
                  (statusCounts["срок оплаты не наступил"] / totalContracts) * 100
                )
              : 0}
            %
          </div>
        ...
        <div className={styles.statCard}>
          <div className={styles.statCardTitle}>Общие расходы</div>
          <div className={styles.statCardValue} style={{ color: COLORS.expense }}>
            {formatCurrency(totalExpense)}
          </div>
          <div className={styles.statCardPercent}>
            по {statusCounts.оплачено} оплаченным договорам
          </div>
        </div>
      </div>
                ...
                <div className={styles.legendControls}>
                  <button
                    className={`${styles.legendButton} ${
                      visibleLines.оплачено ? styles.active : styles.inactive
                    }`}
                    onClick={() => toggleLine("оплачено")}
                  >
                    <span
                      className={styles.legendDot}
                      style={{ backgroundColor: COLORS.оплачено }}
                    ></span>
                    Оплачено
                  </button>
                  ...
                  <button
                    className={`${styles.legendButton} ${
                      visibleLines.просрочено ? styles.active : styles.inactive
                    }`}
                    onClick={() => toggleLine("просрочено")}
                  >
                    <span
                      className={styles.legendDot}
                      style={{ backgroundColor: COLORS.просрочено }}
                    ></span>
                    Просрочено
                  </button>
                  ...
                  <button
                    className={`${styles.legendButton} ${
                      visibleLines["срок оплаты не наступил"]
                        ? styles.active
                        : styles.inactive
                    }`}
                    onClick={() => toggleLine("срок оплаты не наступил")}
                  >
                    <span
                      className={styles.legendDot}
                      style={{
                        backgroundColor: COLORS["срок оплаты не наступил"],
                      }}
                    ></span>
                    Срок не наступил
                  </button>
                ...
                <ResponsiveContainer width="100%" height={380}>
                  ...
                  >
                    ...
                    <Tooltip
                      ...
                      labelFormatter={formatDateLabel}
                    />
                    {visibleLines.оплачено && (
                      <Line
                        type="monotone"
                        dataKey="оплачено"
                        stroke={COLORS.оплачено}
                        strokeWidth={2.5}
                        dot={{ r: 4, fill: COLORS.оплачено }}
                        name="Оплачено"
                      />
                    )}
                    {visibleLines.просрочено && (
                      <Line
                        type="monotone"
                        dataKey="просрочено"
                        stroke={COLORS.просрочено}
                        strokeWidth={2.5}
                        dot={{ r: 4, fill: COLORS.просрочено }}
                        name="Просрочено"
                      />
                    )}
                    {visibleLines["срок оплаты не наступил"] && (
                      <Line
                        type="monotone"
                        dataKey="срок оплаты не наступил"
                        stroke={COLORS["срок оплаты не наступил"]}
                        strokeWidth={2.5}
                        dot={{
                          r: 4,
                          fill: COLORS["срок оплаты не наступил"],
                        }}
                        name="Срок не наступил"
                      />
                    )}
                  </LineChart>
                </ResponsiveContainer>
      ...
      {activeTab === "expenses" && (
        <div className={styles.chartContainer}>
          <div className={styles.chartTitleWithIconCentered}>
            <span>Расходы по времени</span>
          </div>
        ...
        <div className={styles.priceComparisonContainer}>
          <div className={styles.priceFilters}>
            <select
              ...
              onChange={(e) => setSelectedWoodId(Number(e.target.value))}
              className={styles.filterSelect}
            >
              {products.map((p) => (
                <option key={p.wood_id} value={p.wood_id}>
                  {p.wood_type} {p.wood_grade && `(${p.wood_grade})`} -{" "}
                  {p.wood_length}мм
                </option>
              ))}
            </select>
          ...
          {selectedWoodInfo && (
            <div className={styles.selectedWoodInfo}>
              ...
              <div className={styles.woodInfoGrid}>
                <div>
                  <span>Порода:</span> {selectedWoodInfo.wood_type}
                </div>
                <div>
                  <span>Сорт:</span> {selectedWoodInfo.wood_grade || "—"}
                </div>
                <div>
                  <span>Длина:</span> {selectedWoodInfo.wood_length} м
                </div>
                <div>
                  <span>Диаметр:</span> {selectedWoodInfo.wood_diameter} мм
                </div>
                <div>
                  <span>Сечение:</span> {selectedWoodInfo.wood_cross_section}
                </div>
              </div>
            </div>
          )}
          ...
          <div className={styles.priceTable}>
            <div className={styles.priceTableHeader}>
              <div>Поставщик</div>
              <div className={styles.sortable} onClick={handleSortPrice}>
                Цена за м³ (₽){" "}
                {priceSortBy === "price" && (priceSortOrder === "asc" ? "↑" : "↓")}
              </div>
              <div className={styles.sortable} onClick={handleSortQuantity}>
                Доступно (м³){" "}
                {priceSortBy === "quantity" &&
                  (priceSortOrder === "asc" ? "↑" : "↓")}
              </div>
            ...
            {priceComparisonData.map((item, i) => (
              <div key={i} className={styles.priceTableRow}>
                ...
                <div>
                  {item.available_quantity.toLocaleString("ru-RU", {
                    minimumFractionDigits: 2,
                    maximumFractionDigits: 2,
                  })}
                </div>
                <div style={{ fontWeight: "bold" }}>
                  {formatCurrency(item.price * item.available_quantity)}
                </div>
              </div>
            ))}
          </div>
      ...
      {canSeeDeliveriesTab && activeTab === "deliveries" && (
        <div className={styles.deliveriesContainer}>
          <div className={styles.deliveryLegendControls}>
            <button
              className={`${styles.deliveryLegendButton} ${
                deliveryVisibleLines.ожидается
                  ? styles.activeDelivery
                  : styles.inactiveDelivery
              }`}
              onClick={() => toggleDeliveryLine("ожидается")}
            >
              <span
                className={styles.legendDot}
                style={{ backgroundColor: COLORS.ожидается }}
              ></span>
              Ожидается
            </button>
            ...
            <button
              className={`${styles.deliveryLegendButton} ${
                deliveryVisibleLines.доставлено
                  ? styles.activeDelivery
                  : styles.inactiveDelivery
              }`}
            ...
            >
              <span
                className={styles.legendDot}
                style={{ backgroundColor: COLORS.доставлено }}
              ></span>
              Доставлено
            </button>
            ...
            <button
              className={`${styles.deliveryLegendButton} ${
                deliveryVisibleLines.нарушение
                  ? styles.activeDelivery
                  : styles.inactiveDelivery
              }`}
            ...
            >
              <span
                className={styles.legendDot}
                style={{ backgroundColor: COLORS.нарушение }}
              ></span>
              Нарушение
            </button>
          </div>
            ...
            <div className={styles.chartArea}>
              <ResponsiveContainer width="100%" height={400}>
                ...
                >
                  ...
                  <Legend />
                  {deliveryVisibleLines.ожидается && (
                    <Bar dataKey="ожидается" fill={COLORS.ожидается} name="Ожидается" />
                  )}
                  {deliveryVisibleLines.доставлено && (
                    <Bar dataKey="доставлено" fill={COLORS.доставлено} name="Доставлено" />
                  )}
                  {deliveryVisibleLines.нарушение && (
                    <Bar dataKey="нарушение" fill={COLORS.нарушение} name="Нарушение" />
                  )}
                </BarChart>
              </ResponsiveContainer>
            </div>
            ...
            <div className={styles.chartArea}>
              {supplierMonthlyData.length > 0 ? (
                ...
                <div className={styles.noData}>
                  Нет данных о поставках для выбранного поставщика
                </div>
              )}
            </div>
          </div>
